...
class hierarchy_dict {
  ...
  public:
    ...
    hierarchy_dict(It begin, It end) : strings_(begin, end) {
        freeze();
    }
    ...
    uint32_t get_id(std::string_view s) const {
        ...
        if (it != strings_.end() && *it == s)
            return std::distance(strings_.begin(), it);
        return 0;
    }

    std::pair<uint32_t, uint32_t>
    get_prefix_range(std::string_view prefix) const {
        auto it_start =
            std::lower_bound(strings_.begin(), strings_.end(), prefix);
        std::string upper = std::string(prefix);
        if (!upper.empty()) {
            upper.back()++;
        }
        auto it_end = std::lower_bound(strings_.begin(), strings_.end(), upper);
        return {std::distance(strings_.begin(), it_start),
...
struct is_and_i<and_t<L, R>> : std::true_type {};
template <typename> struct is_or_i : std::false_type {};
template <matcher L, matcher R> struct is_or_i<or_t<L, R>> : std::true_type {};

template <typename T>
inline constexpr bool is_not_v = is_not_i<std::remove_cvref_t<T>>::value;
template <typename T>
inline constexpr bool is_and_v = is_and_i<std::remove_cvref_t<T>>::value;
template <typename T>
inline constexpr bool is_or_v = is_or_i<std::remove_cvref_t<T>>::value;
...
template <typename M>
inline constexpr bool is_static_v<not_t<M>> = is_static_v<M>;
template <typename L, typename R>
inline constexpr bool is_static_v<and_t<L, R>> =
    is_static_v<L> && is_static_v<R>;
template <typename L, typename R>
inline constexpr bool is_static_v<or_t<L, R>> =
    is_static_v<L> && is_static_v<R>;
...
constexpr inline class implies_t {
  public:
    ...
    constexpr bool operator()(L const &l, R const &r) const {
        ...
        if constexpr (std::is_same_v<UL, never_t>)
            return true;
        else if constexpr (std::is_same_v<UR, always_t>)
            return true;
        else if constexpr (std::is_same_v<UL, always_t>)
            return false;
        else if constexpr (is_and_v<UL>)
            return (*this)(l.lhs, r) || (*this)(l.rhs, r);
        else if constexpr (is_or_v<UR>)
            return (*this)(l, r.lhs) || (*this)(l, r.rhs);
        else if constexpr (is_or_v<UL>)
            return (*this)(l.lhs, r) && (*this)(l.rhs, r);
        else if constexpr (is_not_v<UL> && is_not_v<UR>)
            return (*this)(r.m, l.m);
        else if constexpr (requires { tag_invoke(*this, l, r); })
            return tag_invoke(*this, l, r);
        else
            return std::is_same_v<UL, UR>;
    }
} implies{};

constexpr inline class negate_t {
  public:
    template <typename M> constexpr auto operator()(M const &m) const {
        using UM = std::remove_cvref_t<M>;
        if constexpr (std::is_same_v<UM, always_t>)
            return never;
        else if constexpr (std::is_same_v<UM, never_t>)
            return always;
        else if constexpr (is_not_v<UM>)
            return m.m;
        else if constexpr (requires { tag_invoke(*this, m); })
...
constexpr inline class simplify_t {
  public:
    template <typename M> constexpr auto operator()(M const &m) const {
        using UM = std::remove_cvref_t<M>;
        if constexpr (std::is_same_v<UM, always_t> ||
                      std::is_same_v<UM, never_t>)
            return m;
        else if constexpr (is_not_v<UM>)
            return negate((*this)(m.m));
        else if constexpr (is_and_v<UM>)
            return (*this)(m.lhs) & (*this)(m.rhs);
        else if constexpr (is_or_v<UM>)
            return (*this)(m.lhs) | (*this)(m.rhs);
        else if constexpr (requires { tag_invoke(*this, m); })
            return tag_invoke(*this, m);
        else
    ...
    template <typename M> constexpr auto operator()(M const &m) const {
        ...
        if constexpr (is_not_v<S>) {
            ...
            if constexpr (is_and_v<Inner>)
                ...
                return (*this)(
                    and_t{(*this)(negate(s.m.lhs)), (*this)(negate(s.m.rhs))});
            else
                return s;
...
template <matcher L, matcher R> constexpr auto operator&(L l, R r) {
    if constexpr (!is_static_v<L> || !is_static_v<R>) {
        if constexpr (std::is_same_v<L, never_t> || std::is_same_v<R, never_t>)
            return never;
        else if constexpr (std::is_same_v<L, always_t>)
            return r;
        else if constexpr (std::is_same_v<R, always_t>)
            return l;
        else
        ...
        auto sl = simplify(l);
        auto sr = simplify(r);
        using TL = decltype(sl);
        using TR = decltype(sr);
        if constexpr (std::is_same_v<TL, never_t> ||
                      std::is_same_v<TR, never_t>)
            return never;
        else if constexpr (std::is_same_v<TL, always_t>)
            return sr;
        else if constexpr (std::is_same_v<TR, always_t>)
            return sl;
        else if constexpr (implies(sl, sr))
            return sl;
        else if constexpr (implies(sr, sl))
            return sr;
        else if constexpr (implies(sl, negate(sr)) || implies(sr, negate(sl)))
            return never;
        else
            return and_t<TL, TR>{sl, sr};
    }
}

template <matcher L, matcher R> constexpr auto operator|(L l, R r) {
    if constexpr (!is_static_v<L> || !is_static_v<R>) {
        if constexpr (std::is_same_v<L, always_t> ||
                      std::is_same_v<R, always_t>)
            return always;
        else if constexpr (std::is_same_v<L, never_t>)
            return r;
        else if constexpr (std::is_same_v<R, never_t>)
            return l;
        else
            return or_t<L, R>{l, r};
    } else {
        ...
        using TL = decltype(sl);
        using TR = decltype(sr);
        if constexpr (std::is_same_v<TL, always_t> ||
                      std::is_same_v<TR, always_t>)
            return always;
        else if constexpr (std::is_same_v<TL, never_t>)
            return sr;
        else if constexpr (std::is_same_v<TR, never_t>)
            return sl;
        else if constexpr (implies(sl, sr))
            return sr;
        else if constexpr (implies(sr, sl))
            return sl;
        else if constexpr (implies(negate(sl), sr) || implies(negate(sr), sl))
            return always;
        else
            return or_t<TL, TR>{sl, sr};
    }
}

template <matcher M> constexpr auto operator!(M m) {
    return negate(simplify(m));
}
...
template <op O, FixedString Field, auto Val> struct field_matcher {
    using is_matcher = void;
    ...
    friend constexpr bool tag_invoke(expr::implies_t, field_matcher const &,
        ...
        if constexpr (!(Field == F2))
            return false;
        else if constexpr (O == op::eq) {
            if constexpr (O2 == op::eq)
                return Val == V2;
            else if constexpr (O2 == op::ne)
                return Val != V2;
            else if constexpr (O2 == op::lt)
                return Val < V2;
            else
                return Val >= V2;
        } else if constexpr (O == op::lt) {
            if constexpr (O2 == op::lt)
                return Val <= V2;
            else if constexpr (O2 == op::ne)
                return Val <= V2;
            else
    ...
    friend constexpr auto tag_invoke(expr::negate_t, field_matcher const &) {
        if constexpr (O == op::eq)
            return field_matcher<op::ne, Field, Val>{};
        if constexpr (O == op::ne)
            return field_matcher<op::eq, Field, Val>{};
        if constexpr (O == op::lt)
            return field_matcher<op::ge, Field, Val>{};
        if constexpr (O == op::ge)
            return field_matcher<op::lt, Field, Val>{};
    }
};

template <FixedString Field, FixedString Path> struct prefix_matcher {
    using is_matcher = void;
    ...
    friend constexpr bool tag_invoke(expr::implies_t, prefix_matcher const &,
        ...
        if constexpr (!(Field == F2))
            ...
            return Path.view().starts_with(P2.view());
    }
};
...
template <size_t CS = 1024> struct table_view {
    size_t rows = 0;
    ...
    size_t num_chunks() const {
        return rows ? (rows + chunk_size - 1) / chunk_size : 0;
    }
    const column_view &get_col(std::string_view name) const {
        auto it = cols.find(name);
        if (it == cols.end())
            throw std::runtime_error("no col: " + std::string(name));
        return it->second;
    }
    bool chunk_summaries_empty() const {
        for (auto &[k, c] : cols)
            if (c.chunk_summaries)
                return false;
        return true;
    }
};
...
struct table {
    ...
    column &get_col(std::string_view n) {
        for (auto &c : cols)
            if (c.name == n)
                return c;
        throw std::runtime_error(std::string("no col: ") + std::string(n));
    }
    const column &get_col(std::string_view n) const {
        for (auto &c : cols)
            if (c.name == n)
                return c;
        throw std::runtime_error(std::string("no col: ") + std::string(n));
    ...
    void add_column_u32(std::string_view n, uint32_t d = 0) {
        cols.push_back({n, std::vector<uint32_t>(rows, d), {}, {}});
    }
    void add_column_f32(std::string_view n, float d = 0.f) {
        cols.push_back({n, {}, std::vector<float>(rows, d), {}});
    }

    void add_column_hst(std::string_view n,
                        const std::vector<std::string> &vals) {
        hierarchy_dict dict(vals.begin(), vals.end());
        std::vector<uint32_t> ids(rows);
        for (size_t i = 0; i < rows && i < vals.size(); ++i)
            ids[i] = dict.get_id(vals[i]);
    ...
    template <size_t CS = 1024> table_view<CS> to_view() const {
        table_view<CS> tv;
        tv.rows = rows;
        ...
        for (const auto &c : cols) {
            ...
            cv.chunk_summaries =
                chunk_summaries.empty() ? nullptr : chunk_summaries.data();
            tv.cols[c.name] = cv;
        }
        tv.dicts = &dicts;
        return tv;
...
template <typename L, typename R>
inline constexpr bool needs_scratch<expr::and_t<L, R>> = true;
template <typename L, typename R>
inline constexpr bool needs_scratch<expr::or_t<L, R>> = true;
template <typename M>
inline constexpr bool needs_scratch<expr::not_t<M>> = needs_scratch<M>;
} // namespace detail
...
struct engine {
    ...
    struct exec_ctx {
        ...
        explicit exec_ctx(size_t num_words)
            : result(num_words, 0), chunk_buf(num_words, 0),
              scratch(num_words, 0) {}
    };
    ...
    template <size_t CS>
    static mask_t execute(const table_view<CS> &t,
                          expr::matcher auto const &matcher) {
    ...
    template <size_t CS>
    static mask_t execute_incremental(exec_ctx &ctx, incremental_result &ir,
        ...
        eval_from_view(t, query, from, count, ctx.chunk_buf.data(),
                       ctx.scratch.data(), delta_words);

        simd::bor(ir.result.data() + from_word, ctx.chunk_buf.data(),
                  delta_words);
    ...
    template <size_t CS>
    static expr::field_in semi_join(const table_view<CS> &t,
                                    expr::matcher auto const &q,
                                    std::string_view fk_field) {
        return {fk_field, std::make_shared<mask_t>(execute(t, q))};
    }
    ...
    static double aggregate(const table &t, expr::matcher auto const &q,
        ...
        if (!c.f32.empty())
            return agg_impl(mask, c.f32, op, t.rows);
        if (!c.u32.empty())
            return agg_impl(mask, c.u32, op, t.rows);
        if (!c.u64.empty())
            return agg_impl(mask, c.u64, op, t.rows);
        return 0.0;
    }

    template <size_t CS>
    static double aggregate(const table_view<CS> &t,
                            expr::matcher auto const &q, std::string_view col,
                            agg op) {
        auto mask = execute(t, q);
        auto &c = t.get_col(col);
        if (c.f32)
            return agg_impl_view(mask, c.f32, op, t.rows);
        if (c.u32)
            return agg_impl_view(mask, c.u32, op, t.rows);
        if (c.u64)
            return agg_impl_view(mask, c.u64, op, t.rows);
        return 0.0;
    }
  ...
  private:
    ...
    static void execute_core(mask_t &result, mask_t &chunk_buf, mask_t &scratch,
        ...
        if (req == 0 || t.chunk_summaries_empty()) {
            eval_from_view(t, query, 0, t.rows, result.data(), scratch.data(),
                           num_words);
        } else {
            for (size_t c = 0; c < t.num_chunks(); ++c) {
                auto &col = t.get_col("mask");
                uint64_t summary =
                    col.chunk_summaries ? col.chunk_summaries[c] : 0;
                if ((summary & req) != req)
                    continue;
                size_t start = c * t.chunk_size;
                size_t count = std::min(t.chunk_size, t.rows - start);
                size_t cnum_words = simd::num_words(count);
                eval_from_view(t, query, start, count, chunk_buf.data(),
                               scratch.data(), cnum_words);
                simd::bor(result.data() + start / 64, chunk_buf.data(),
                          cnum_words);
            }
        }
    ...
    template <typename M> static uint64_t extract_bits(M const &) { return 0; }

    template <op O, FixedString Field, auto Val>
    static uint64_t extract_bits(field_matcher<O, Field, Val> const &) {
        if constexpr (O == op::eq && Field == FixedString("mask"))
    ...
    static uint64_t extract_bits(expr::and_t<L, R> const &a) {
    ...
    static double agg_impl(const mask_t &mask, const std::vector<T> &data,
        ...
        for (size_t i = 0; i < rows; ++i) {
            if (!simd::test(mask.data(), i))
                continue;
            double v = (double)data[i];
            if (op == agg::sum || op == agg::mean)
                r += v;
            else if (op == agg::max) {
                if (first || v > r)
                    r = v;
    ...
    template <typename T>
    static double agg_impl_view(const mask_t &mask, const T *data, agg op,
                                size_t rows) {
        if (op == agg::count)
            return (double)simd::popcount(mask.data(), mask.size());
        double r = 0;
    ...
    template <size_t CS>
    static void eval_from_view(const table_view<CS> &, expr::always_t, size_t,
                               size_t count, uint64_t *out, uint64_t *,
                               size_t num_words) {
    ...
    template <size_t CS>
    static void eval_from_view(const table_view<CS> &, expr::never_t, size_t,
                               size_t, uint64_t *out, uint64_t *,
                               size_t num_words) {
        std::fill_n(out, num_words, 0);
    }

    template <size_t CS, expr::matcher L, expr::matcher R>
    static void eval_from_view(const table_view<CS> &t,
                               expr::and_t<L, R> const &a, size_t s, size_t c,
                               uint64_t *out, uint64_t *scratch,
                               size_t num_words) {
        eval_from_view(t, a.lhs, s, c, out, scratch, num_words);
        if constexpr (detail::needs_scratch<R>) {
            mask_t tmp(num_words);
            eval_from_view(t, a.rhs, s, c, scratch, tmp.data(), num_words);
        } else {
    ...
    template <size_t CS, expr::matcher L, expr::matcher R>
    static void eval_from_view(const table_view<CS> &t,
                               expr::or_t<L, R> const &o, size_t s, size_t c,
                               uint64_t *out, uint64_t *scratch,
                               size_t num_words) {
        eval_from_view(t, o.lhs, s, c, out, scratch, num_words);
        if constexpr (detail::needs_scratch<R>) {
            mask_t tmp(num_words);
            eval_from_view(t, o.rhs, s, c, scratch, tmp.data(), num_words);
        } else {
    ...
    static void eval_from_view(const table_view<CS> &t, expr::not_t<M> const &n,
                               size_t s, size_t c, uint64_t *out,
                               uint64_t *scratch, size_t num_words) {
    ...
    static void eval_from_view(const table_view<CS> &t,
                               field_matcher<O, Field, Val> const &,
                               size_t start, size_t count, uint64_t *out,
                               uint64_t *, size_t num_words) {
        auto &col = t.get_col(Field.view());
        if (col.u32) {
            if constexpr (O == op::eq)
                simd::cmp_eq(out, col.u32 + start, (uint32_t)Val, count);
            if constexpr (O == op::ne)
                simd::cmp_ne(out, col.u32 + start, (uint32_t)Val, count);
            if constexpr (O == op::lt)
                simd::cmp_lt(out, col.u32 + start, (uint32_t)Val, count);
            if constexpr (O == op::ge)
                simd::cmp_ge(out, col.u32 + start, (uint32_t)Val, count);
        } else if (col.f32) {
            if constexpr (O == op::eq)
                simd::cmp_eq(out, col.f32 + start, (float)Val, count);
            if constexpr (O == op::ne)
                simd::cmp_ne(out, col.f32 + start, (float)Val, count);
            if constexpr (O == op::lt)
                simd::cmp_lt(out, col.f32 + start, (float)Val, count);
            if constexpr (O == op::ge)
                simd::cmp_ge(out, col.f32 + start, (float)Val, count);
        } else if (col.u64) {
            if constexpr (O == op::eq)
                simd::cmp_eq(out, col.u64 + start, (uint64_t)Val, count);
            if constexpr (O == op::ne)
                simd::cmp_ne(out, col.u64 + start, (uint64_t)Val, count);
            if constexpr (O == op::lt)
                simd::cmp_lt(out, col.u64 + start, (uint64_t)Val, count);
            if constexpr (O == op::ge)
                simd::cmp_ge(out, col.u64 + start, (uint64_t)Val, count);
        } else {
            std::fill_n(out, num_words, 0);
        }
    }
    ...
    static void eval_from_view(const table_view<CS> &t,
                               prefix_matcher<Field, Path> const &,
                               size_t start, size_t count, uint64_t *out,
                               uint64_t *, size_t num_words) {
        if (!t.dicts) {
            std::fill_n(out, num_words, 0);
        ...
        simd::cmp_fill(out, col.u32 + start, 0, count,
                       [=](uint32_t v, auto) { return v >= low && v < high; });
    }
