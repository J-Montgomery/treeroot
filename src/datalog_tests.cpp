...
TEST(Engine, CompoundQuery) {
    ...
    t.add_column_u32("s");
    t.get_col("s").u32 = {1, 2, 1, 2, 1};
    auto r = engine::execute(t, field_matcher<op::lt, "id", 13>{} &
                                    field_matcher<op::eq, "s", 2>{});
    EXPECT_TRUE(simd::test(r.data(), 1));
...
TEST(Engine, SemiJoin) {
    table users(3);
    table logs(4);
    logs.add_column_u32("uid");
    logs.get_col("uid").u32 = {0, 0, 2, 5};
...
TEST(Engine, SemiJoinCompound) {
    ...
    for (size_t i = 0; i < 100; ++i)
        players.get_col("team").u32[i] = (i % 3) + 1;

    table pets(100);
    pets.add_column_u32("oid");
    pets.add_column_u32("sp");
    for (size_t i = 0; i < 100; ++i) {
        pets.get_col("oid").u32[i] = i % 100;
        pets.get_col("sp").u32[i] = (i % 5) + 1;
    }
    auto join =
        engine::semi_join(players, field_matcher<op::eq, "team", 2>{}, "oid");
    auto query = field_matcher<op::eq, "sp", 3>{} & join;
    auto r = engine::execute(pets, query);
    for (size_t i = 0; i < 100; ++i) {
        bool expect =
            (pets.get_col("sp").u32[i] == 3) &&
...
TEST(Engine, SemiNaiveChain) {
    table t(100);
    t.add_column_u32("pid", 999);
    for (uint32_t i = 1; i < 5; ++i)
        t.get_col("pid").u32[i] = i - 1;
    auto rm =
        engine::semi_naive_join(t, field_matcher<op::eq, "id", 0>{}, "pid");
...
TEST(Engine, Aggregate) {
    table t(1000);
    t.add_column_u32("team");
    t.add_column_f32("hp");
    for (size_t i = 0; i < 500; ++i) {
        t.get_col("team").u32[i] = 1;
        t.get_col("hp").f32[i] = 100.f;
    }
    for (size_t i = 500; i < 1000; ++i) {
        t.get_col("team").u32[i] = 2;
        t.get_col("hp").f32[i] = 50.f;
    }
...
TEST(Engine, BloomFilterSkip) {
    ...
    for (int i = 0; i < 5; ++i) {
        t.set_tag(5 * 1024 + i, 0);
        t.set_tag(5 * 1024 + i, 1);
    }
    auto q = field_matcher<op::eq, "mask", 3>{} &
             field_matcher<op::lt, "dur", 150>{};
...
TEST(Engine, BloomFilterDNF) {
    table t(10240, 1024);
    t.add_column_u32("dur", 100);
    t.add_column_u32("sp", 0);
    for (int i = 0; i < 5; ++i) {
        t.set_tag(i, 0);
        t.set_tag(i, 1);
    }
    auto q = or_t{and_t{field_matcher<op::eq, "mask", 3>{},
                        field_matcher<op::lt, "dur", 150>{}},
                  and_t{field_matcher<op::eq, "mask", 3>{},
                        field_matcher<op::eq, "sp", 0>{}}};
    auto r = engine::execute(t, q);
    EXPECT_EQ(simd::popcount(r.data(), r.size()), 5u);
}
...
TEST(Engine, Contradiction) {
    table t(1000);
    t.add_column_u32("v", 10);
    auto q =
        field_matcher<op::eq, "v", 10>{} & field_matcher<op::ne, "v", 10>{};
    static_assert(std::is_same_v<decltype(q), never_t>);
    EXPECT_EQ(
        simd::popcount(engine::execute(t, q).data(), simd::num_words(1000)),
        0u);
}

TEST(Engine, Tautology) {
    table t(1000);
    t.add_column_f32("hp", 50.f);
    auto q =
        field_matcher<op::lt, "hp", 100>{} | field_matcher<op::ge, "hp", 100>{};
...
TEST(Engine, EmptyJoin) {
    table p(100);
    p.add_column_u32("team", 0);
    table pets(100);
    pets.add_column_u32("oid", 0);
    auto sj = engine::semi_join(p, field_matcher<op::eq, "team", 999>{}, "oid");
    EXPECT_EQ(
        simd::popcount(engine::execute(pets, sj).data(), simd::num_words(100)),
        0u);
}

TEST(Engine, NegativeJoin) {
    table p(10);
    p.add_column_u32("team");
    for (size_t i = 0; i < 10; ++i)
        p.get_col("team").u32[i] = (i < 5) ? 1 : 2;
    auto nj = engine::semi_join(p, !field_matcher<op::eq, "team", 2>{}, "oid");
    table pets(10);
    pets.add_column_u32("oid");
    for (size_t i = 0; i < 10; ++i)
        pets.get_col("oid").u32[i] = i;
    auto r = engine::execute(pets, nj);
    for (size_t i = 0; i < 5; ++i)
        EXPECT_TRUE(bit_at(r, i));
    for (size_t i = 5; i < 10; ++i)
        EXPECT_FALSE(bit_at(r, i));
}
...
TEST(Engine, TableViewCompound) {
    ...
    auto q = field_matcher<op::lt, "id", 40>{} &
             field_matcher<op::ge, "val", 20.0f>{};
...
TEST(Datalog, TCDisconnected) {
    ...
    for (int i : {0, 1, 2, 5, 6, 7})
        EXPECT_TRUE(bit_at(p.get_bits("reach"), i)) << i;
    for (int i : {3, 4})
        EXPECT_FALSE(bit_at(p.get_bits("reach"), i)) << i;
}
...
TEST(Datalog, SimpleNegation) {
    ...
    p.evaluate();
    for (int i : {0, 1, 2})
        EXPECT_TRUE(bit_at(p.get_bits("reach"), i));
    EXPECT_FALSE(bit_at(p.get_bits("reach"), 3));
    for (int i : {0, 1, 2})
        EXPECT_FALSE(bit_at(p.get_bits("unreach"), i));
...
TEST(Datalog, ThreeStrata) {
    ...
    for (int i = 0; i < 5; ++i)
        EXPECT_TRUE(bit_at(p.get_bits("fin"), i));
    for (int i = 5; i < 10; ++i)
        EXPECT_FALSE(bit_at(p.get_bits("fin"), i));
}

TEST(Engine, HierarchyDict) {
    table t(4);
    t.add_column_hst("path", {"sensor/temp/1", "sensor/voltage", "motor/rpm",
                              "sensor/temp/2"});
...
TEST(Datalog, PureIDBIntersection) {
    ...
    p.add_rule({"A",
                ...
                {{"nodes", {"X"}}},
                [](const table &t) {
                    return engine::execute(t, field_matcher<op::lt, "id", 4>{});
                },
                0});
    p.add_rule({"B",
                {"X"},
                {{"nodes", {"X"}}},
                [](const table &t) {
                    return engine::execute(t, field_matcher<op::ge, "id", 2>{});
                },
                0});
...
//         [](const table& t) {
//             return engine::execute(t, field_matcher<op::ge,
//             FixedString("level"), 10u>{});
//         },
//         0
//     });
...
TEST(Datalog, RegexPathMatcher) {
    ...
    auto edges = make_edges({
        ...
        {2, 3}, // Match: 0 -> 1 -> 2 -> 3
        {10, 11},
        {11, 12}, // No Match: too short
        {20, 21},
        {21, 22},
        {22, 23},
        {23, 24} // Match: 20 -> 21 -> 22 -> 23
    });
    ...
    EXPECT_FALSE(simd::test(p.get_bits("S3").data(),
                            12)); // 10 only reached 12 (2 steps)
    EXPECT_TRUE(simd::test(p.get_bits("S3").data(), 23)); // 20 reached 23
    EXPECT_TRUE(
        simd::test(p.get_bits("S3").data(),
                   24)); // 20 reached 24 (it's also a valid 3-step end from 21)
}
