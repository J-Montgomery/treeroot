...
static void BM_Execute(benchmark::State &st) {
    ...
    auto q =
        field_matcher<op::lt, "id", 100>{} & field_matcher<op::eq, "s", 1>{};
    for (auto _ : st)
        benchmark::DoNotOptimize(engine::execute(t, q));
...
static void BM_ExecuteExternalMemory_Incremental(benchmark::State &st) {
    ...
    auto q =
        field_matcher<op::lt, "id", 100>{} & field_matcher<op::eq, "s", 1>{};
    for (auto _ : st)
        benchmark::DoNotOptimize(engine::execute_incremental(ctx, ir, tv, q));
    st.SetItemsProcessed(st.range(0) * st.iterations());
}
BENCHMARK(BM_ExecuteExternalMemory_Incremental)
    ->RangeMultiplier(10)
    ->Range(1000, 1000000);
...
static void BM_ExecuteExternalMemory(benchmark::State &st) {
    ...
    auto q =
        field_matcher<op::lt, "id", 100>{} & field_matcher<op::eq, "s", 1>{};
    for (auto _ : st)
        benchmark::DoNotOptimize(engine::execute(tv, q));
    st.SetItemsProcessed(st.range(0) * st.iterations());
}
...
static void BM_Datalog_Negation(benchmark::State &st) {
    ...
    for (auto _ : st) {
        ...
        p.add_rule({"reach", {"X"}, {{"reach", {"Y"}}, {"edge", {"Y", "X"}}}});
        p.add_rule(
            {"unreach", {"X"}, {{"nodes", {"X"}}, {"reach", {"X"}, true}}});
        p.evaluate();
...
static void BM_BoundingBox(benchmark::State &st) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        world.get_col("y").f32[i] =
            static_cast<float>((i / 1000) % 1000) * 0.1f;
    }
...
static void BM_Bloom_Crossover(benchmark::State &st) {
    ...
    size_t num_polluted_chunks =
        static_cast<size_t>((n / chunk_size) * pollution_pct);
    size_t stride =
        (num_polluted_chunks > 0) ? (n / chunk_size) / num_polluted_chunks : n;
...
BENCHMARK(BM_Datalog_Pathfinding_Long)
    ->Arg(100)
    ->Arg(1000)
    ->Arg(10000)
    ->Arg(100000)
...
//         p.add_rule({"state_b", {"Next"}, {{"state_a", {"Curr"}}, {"step",
//         {"Curr", "Next"}}}}); p.add_rule({"state_c", {"Next"}, {{"state_b",
//         {"Curr"}}, {"step", {"Curr", "Next"}}}}); p.add_rule({"state_d",
//         {"Next"}, {{"state_c", {"Curr"}}, {"step", {"Curr", "Next"}}}});
...
// }
// BENCHMARK(BM_Datalog_Regex_Propagation)->RangeMultiplier(10)->Range(1000,
// 100'000);
    ...
    for (auto _ : st) {
        ...
        while (changed) {
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                // State 1 -> State 2
                if (simd::test(s1.data(), i) &&
                    !simd::test(s2.data(), target)) {
                    simd::set(s2.data(), target);
                    changed = true;
                }
                // State 2 -> State 3
                if (simd::test(s2.data(), i) &&
                    !simd::test(s3.data(), target)) {
...
//             engine::eval_into_view(tv, f1, 0, n, s2.data(), scratch.data(),
//             nw);
...
//             engine::eval_into_view(tv, f2, 0, n, s3.data(), scratch.data(),
//             nw);
//         }
//         benchmark::DoNotOptimize(s3.data());
//     }
